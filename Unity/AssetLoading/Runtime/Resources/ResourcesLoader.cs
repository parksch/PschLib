...
namespace PschLib.AssetLoading.Resources
{
...
#endif
    {
        ...
        private readonly Dictionary<AssetKey, PendingLoad> pendingLoads = new Dictionary<AssetKey, PendingLoad>();
        ...
        public TAsset Load<TAsset>(string path) where TAsset : Object
        {
            ...
            if (cache.TryAcquire<TAsset>(path, out var cachedAsset))
            {
        ...
        public async UniTask<TAsset> LoadAsync<TAsset>(string path, CancellationToken cancellationToken = default) where TAsset : Object
        {
            ...
            if (!cache.TryAcquire<TAsset>(path, out var asset))
            {
                return null;
            }
        ...
        public void Release<TAsset>(string path) where TAsset : Object
        {
            ...
            var result = cache.Release<TAsset>(path);
        ...
        public void Unload<TAsset>(string path) where TAsset : Object
        {
            ...
            var result = cache.Unload<TAsset>(path, out var asset);
        ...
        {
            ValidateMainThread();
            var pendingCount = pendingLoads.Count;
            var activeCount = cache.ActiveCount;
        ...
        private async UniTask LoadPendingAsync<TAsset>(string path, AssetKey key, PendingLoad pendingLoad, int generation) where TAsset : Object
        {
            try
            {
                ...
                if (!cache.TryGet<TAsset>(path, out var cachedAsset))
                {
                    cache.AddUnused(path, asset);
                    cachedAsset = asset;
                }
